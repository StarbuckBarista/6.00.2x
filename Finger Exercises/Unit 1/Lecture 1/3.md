...
Use +, -, / signs to indicate addition, subtraction, and division. Explicitly indicate multiplication with a \* (ie say "6\*n" rather than "6n"). Indicate exponentiation with a caret (^) (ie "n^4" for ***n<sup>4</sup>***). Indicate base-2 logarithms with the word log2 followed by parenthesis (ie "log2(n)").

What this all means is if an answer is, for example, ***O(n<sup>4</sup>)***, please simply write "n^4" in the box.
